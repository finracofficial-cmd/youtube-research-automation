# video — 編集（工程C）

台本と素材から動画を組む Remotion プロジェクトと、参照動画の編集リズムを測る道具。

## 構成

```
video/
  src/            Remotion のコンポーネント
    Root.tsx        コンポジション登録。尺は props から自動計算
    Documentary.tsx 本体。画→章カード→テロップ→字幕の順に重ねる
    schema.ts       props の zod スキーマ
    theme.ts        色とフォント
    components/     KenBurns / Telop / Subtitle / ChapterCard
  build_props.py  台本 → props.json
  analyze_edit.py 動画 → カット点とリズムの統計
  public/         素材（shots/ に画像を置く）
```

## 使い方

```bash
# 1) 台本から props.json を組む
python3 video/build_props.py drafts/oparts_v1.txt --duration 900 --kind bundle --claims 5

# 2) 素材を public/shots/ に 000.jpg, 001.jpg ... と置く

# 3) 確認（ブラウザのスタジオが開く）
cd video && npm run studio

# 4) レンダリング
cd video && npx remotion render src/index.ts Documentary out/video.mp4 --props=props.json
```

字幕は台本の文字数按分で割り付ける。ナレーション音声ができたら
`--narration` で渡して、実測の尺に合わせる。

## 参照動画の編集リズムを測る

```bash
python3 video/analyze_edit.py <動画> --threshold 0.30 --frames-dir frames/ --out stats.json
```

カット点を検出して、1分あたりのカット数と1カットの長さを出す。
その実測値を `build_props.py --shot-sec` に入れれば、リズムを合わせられる。

**ffmpeg のシーン検出は輝度平面しか見ない。** 明るさが近い画どうしの切り替わりは、
色がまったく違っても検出されない（ffmpeg の `red` と `green` はどちらも Y=81 で差がゼロ）。
暗い写本画像が続く構成では取りこぼすので、`--threshold` を下げて調整する。
テストを書いたとき、まさにこれで落ちた。

参照動画から取り出したフレームは**寸法の計測用**。
映像そのものは制作者の著作物なので、自分の動画には使わない。
素材はPD/CCのものを別途集める（元チャンネルもそうしている）。

## オーバーレイ部品

背景（Ken Burns）と字幕だけでは、参照チャンネルの画面は作れない。
上に載る構造化された部品が要る。

| 部品 | props | 用途 |
|---|---|---|
| `SourceLabel` | `sourceLabels` | 左上に出しっぱなしの出典表記。**CC BY の表示義務もこれで満たす** |
| `QuoteCard` | `quoteCards` | 見出し／原文／訳／出典 の4段パネル。左右中央に配置できる |
| `ChipStack` | `chipStacks` | `a ⇄ o` のような小さなトークンの縦積み。対応関係の提示 |
| `CardRow` | `cardRows` | 横並びカード。`dimmed` で「今回は使わない」を表す |
| `DocumentCard` | `documentCards` | 論文の1ページ目を紙色で再現。出典を「見せる」 |
| `BackgroundPlate` | `backgroundDim` | 背景を落としてビネット。文字を載せる下地 |

重ね順は 画 → 下地 → カード類 → 出典ラベル → 章カード → テロップ → 字幕。
**出典ラベルを他のカードより上に置いている**のは、表示義務のあるものを隠さないため。

全部 HTML/CSS で、映像エフェクトではない。Remotion の得意分野なので、
新しい型の部品が要るときは同じ要領で足せる。

## 検証済みのこと

- `npx tsc --noEmit` が通る
- 静止画レンダリング成功（字幕の日本語表示とタイミングを目視確認）
- 動画レンダリング成功（241フレーム / 8秒 / 3.2MB）
- カット検出を合成動画（既知のカット2箇所）で検証（tests/test_edit_analysis.py）
- オーバーレイ部品を実素材の上に重ねて描画確認（`demo.json`）。
  引用カード・出典ラベル・チップ・カード列の減光状態まで目視で確認済み

## 未着手

- ナレーション音声（TTS）の生成と、それに合わせた字幕の再タイミング
- サムネイル生成
- 背景のピラーボックス（画を内側に嵌めて左右を暗く落とす表現）
- 図解・グラフの部品（数値を示すカット用）

## 台本からの自動組版（autolayout.py）

台本自身の言い回しが、出すべき部品を示している。修辞の型をそのまま規則にした。

| 台本の書き方 | 出る部品 |
|---|---|
| `2006年にネイチャーへ載る` | `DocumentCard`（年号＋掲載誌を抜く） |
| `主張はこうだ。` → 次の文 | `QuoteCard`（次の1〜2文を中身に） |
| `5つを並べた` / `四つに分かれる` | `CardRow`（個数ぶんのカード） |
| `ただし、決着はしていない` | `QuoteCard`（逆側に留保として） |
| 1文に数字が3つ以上 | `ChipStack` |
| それ以外 | 何も出さない（画と字幕だけ） |

```bash
python3 video/build_props.py drafts/oparts_v1.txt --duration 900 --kind bundle \
  --claims 5 --manifest video/public/shots/manifest.json
# -> 字幕320枚 / カット112 / 章5 / オーバーレイ10件 / 出典ラベル7件
```

**出典ラベルは manifest から自動生成される。** 素材の作者とライセンスが各カットの
表示中ずっと左上に出るので、CC BY の表示義務が構造的に満たされる。

密度は抑える方に倒している。15分でオーバーレイ10件。出しすぎると読めない。
`schedule()` が最低間隔 1.2秒 を守り、画面中央を占める部品は同時に1つだけにする。

## 描画するときの注意（実際に事故った）

**Remotion は `staticFile()` をプロセスの作業ディレクトリ基準で解決する。**
リポジトリ直下から `npx --prefix video remotion ...` と呼ぶと `public/` を見失い、
**エラーを出さずに画像が全部消えた黒い動画が出来上がる。**
`video/render.sh` 経由で叩けば必ず正しい場所で動く。

```bash
cd video && PROPS=props.json ./render.sh render src/index.ts Documentary out/video.mp4
```

## アニメーションの実測

「動いているつもり」を避けるため、フレーム差で測った（0=静止、255=全面変化）。

### Ken Burns（不透明度が安定した区間）

| 比較 | 平均差 |
|---|---|
| 2.8秒差 | 17 |
| 2.8秒差 | 18 |
| 5.7秒差 | 22 |
| 同一フレーム同士（対照） | **0** |

### 実際に描画した動画（10秒）

| 測り方 | 結果 | 意味 |
|---|---|---|
| 連続フレーム間 | **0.00** | 1フレームあたりの変化が1/255未満＝カクつきが無い |
| 0.5秒 vs 9.5秒 | **36** | 9秒かけて確実に移動している |

**この2つが両立しているのが正しい状態。** 連続フレーム間が大きいとガタつき、
累積が0だと静止画。緩やかなパン・ズームとして機能している。

### 注意: 背景の落とし込みが動きを潰す

`backgroundDim: 0.5` を掛けると、同じ移動量が 17〜22 → **1〜3** まで圧縮された。
暗くするほど動きが見えなくなる。文字の可読性とのトレードオフなので、
題材の画が明るいときは `backgroundDim` を 0.3 程度まで下げたほうがよい。

## 台本が変わると何が変わるか

同じパイプラインに別の台本を通した結果。

| | オーパーツ（束ね15分） | ナスカ（旗艦20分） |
|---|---|---|
| 字幕 | 320枚 | 400枚 |
| カット | 112 | 150 |
| 章 | 5 | 4 |
| 引用カード | 5 | 6 |
| チップ | 1 | 3 |
| カード列 | 2 | 2 |
| 論文カード | 2 | **0** |
| `backgroundDim` | 0.5 | 0.5 |

中身:
- オーパーツ … 列挙「この動画では、語られている5つを並べた」→ **5枚**
- ナスカ … 列挙「話は4つに分かれる」→ **4枚**、引用「ライヘの結論はこうだった」

**部品の種類・重ね順・文体・Ken Burnsの規則は固定。中身と枚数だけが台本から決まる。**

ナスカで論文カードが0件なのは、規則が「年号＋掲載誌」の形しか拾えていないため。
台本には論文への言及があるので、これは規則側の取りこぼしで、拡張の余地がある。

## 密度の追い込み（実測ベース）

参考chのストーリーボード324フレームを実測したところ、構造化オーバーレイの被覆率は
**67.0%**、何も無いフレームは9.0%だった（`analysis/reference_edit_density.md`）。
当初は「出しすぎると読めない」と考えて1画面1部品・最低間隔1.2秒に絞っており、
**6.6%しか出ていなかった。設計判断の誤り。**

### ゾーン制への作り替え

部品は `left` / `right` / `center` / `lower` / `corner` のいずれかを占める。

- **同じゾーンは同時に1つまで。違うゾーンなら同時に出す**
- 画面全体の同時表示は `MAX_CONCURRENT = 3` まで
- 数値系は件数が多いので、`right → corner → left` と交互に振り分ける

### 「次が来るまで保持する」

1文ぶんで消すと画面が寂しくなる。実物では、パネルは話題が続く間ずっと出ていて、
次の話題で差し替わっている。被覆率67%はこの**持続**によるもので、
部品を機関銃のように出しているわけではない。`hold()` が同じゾーンの次の部品まで
（`MAX_HOLD = 16秒` を上限に）伸ばす。

### 到達度（同じ検出器で自分の出力を測った）

| 閾値 | 参考ch（324枚） | 改修前 | 改修後（20枚） |
|---|---|---|---|
| 0.05以上 | 81.2% | — | **80.0%** |
| 0.10以上 | **67.0%** | 6.6% | **60.0%** |
| 0.20以上 | 51.5% | — | 40.0% |
| 完全に0 | 9.0% | 93% | 20.0% |

段階: 6.6% → 23.8%（ゾーン制）→ 45.4%（保持）→ 54.5%（ゾーン分散）→ 実測60.0%。

**自分のサンプルは20枚しかない**ので精度は粗い。参考chは324枚で測っている。
厳しい閾値（0.20）と「完全に0」では依然として差がある。

## 追加した部品

| 部品 | 出る条件 |
|---|---|
| `StatCallout` | 数値＋単位（「7メートル」「600枚」） |
| `PortraitCard` | 人名＋役割語（考古学者・提督など）。年号があれば添える |
| `DataChart` | パーセント表記。読み値と減衰曲線 |
| `TimelineBar` | 1文に年号が2つ以上 |
| `GlyphHero` | 鉤括弧に入った1〜2文字 |
| `MatrixGrid` | 「〜ページある」「〜点に及ぶ」のような総量 |

既存の引用カード／チップ／カード列／論文カードと合わせて10種。

## 密度と正確さのトレードオフ（実測）

自動抽出した中身を点検したところ、**画面に出る事実誤り**が複数見つかった。

| 抽出結果 | 問題 |
|---|---|
| 値=`1世紀` ← 「沈没船の年代は**紀元前**1世紀」 | 接頭辞が落ちて**2000年ずれた別の事実**になる |
| 名前=`ハプグッド` 役割=`学者` ← 「地質**学者ではない**」 | 否定文から役割を取り、**逆の意味**になる |
| 名前=`オスマン` ← 「**オスマン帝国**の提督ピリ・レイス」 | 組織名を人名として拾う |
| ラベル=`…という点火プ` | 語の途中で切れる |
| 値=`1人`（潜水夫が1人死亡）/ `2枚`（2枚の歯車） | 大書きに値しない数をノイズとして出す |

すべて修正し、回帰テストを置いた。ただし**副作用がある**。

| | 修正前 | 修正後 |
|---|---|---|
| 被覆率 | 54.5% | **41.4%** |
| 人物カード | 1件（誤り） | **0件** |

**ノイズを落とすと密度が落ちる。** 参考chの67%に寄せようとルールを緩めると、
今度は画面に嘘が出る。このチャンネルの価値は正確さなので、
**密度より正確さを優先する**のが正しい判断だと考えている。

人物カードが0件になったのは、安全側に倒した結果として正しい挙動だが、
「人名を正しく取る」規則がまだ書けていないという意味でもある。
日本語の人名抽出は、組織名・役割語・否定と紛れやすい。

### 運用上の含意

`check` と同じで、**自動組版が通ることは体裁の保証であって、事実の保証ではない**。
公開前に、少なくとも以下は人が見る必要がある:

- `stats` の値（接頭辞・単位の取りこぼし）
- `portraits` の氏名（組織名との混同）
- `charts` の読み値（文脈と合っているか）
