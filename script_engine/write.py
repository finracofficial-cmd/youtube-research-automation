...
- 同じ単位の値が資料にそろっているなら、1つずつ別の文で並べる。
  「Aは25キロ、Bは250キロ」のように1文に押し込まず、
  「Aは25キロだ。」「Bは250キロある。」と分けて書く。
  画面はこれを並べて棒で見せる。1文にまとまっていると幅（レンジ）と
  区別がつかず、並べられない（実測で比較できる箇所が3本の台本すべてで
  2件ずつしか無かった）。資料に無い値を作って並べてはいけない。
- 一次資料に当たった結果を語る口調にする。
